...
class HorizontalDotedProgressBar : View {
    //actual dot radius
    ...
    private fun createDot(canvas: Canvas, paint: Paint) {
        ...
        for (i in 0 until mDotAmount) {
            if (i == mDotPosition) {
                canvas.drawCircle((10 + i * 20).toFloat(), mBounceDotRadius.toFloat(),
                        mBounceDotRadius.toFloat(), paint)
            } else {
                canvas.drawCircle((10 + i * 20).toFloat(),
                        mBounceDotRadius.toFloat(), mDotRadius.toFloat(), paint)
            }
        }
    }
